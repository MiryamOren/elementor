	...
	const handleSave = async () => {
		...
		await saveElementAsComponent( element.current, componentName.trim(), {
			onSuccess: ( result: ComponentCreateResponse ) => {
				setResultNotification( {
					...
					message: __( 'Component saved successfully as: %1$s (ID: %2$s)', 'elementor' )
						.replace( '%1$s', componentName ).replace( '%2$s', result.component_id.toString() ),
					type: 'success',
				} );
			...
			onError: ( error: unknown ) => {
				const errorMessage =
					error instanceof Error ? error.message : __( 'Failed to save component. Please try again.', 'elementor' );
				setResultNotification( {
					show: true,
