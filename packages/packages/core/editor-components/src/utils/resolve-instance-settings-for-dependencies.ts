...
export function resolveInstanceSettingsForDependencies( {
	...
	const overridableByPropKey = buildOverridableByPropKey(
		overridableProps,
		elementId
	);
	...
	for ( const [ key, val ] of Object.entries( elementSettings ) ) {
		...
		if ( overridableInfo ) {
			const matchingOverride = getMatchingOverride(
				overrides,
				overridableInfo.overrideKey
			);
...
type OverridableByPropKey = Record<
	string,
	{ overrideKey: string; originValue: PropValue }
>;

function buildOverridableByPropKey(
	overridableProps: OverridableProps,
	elementId: string
): OverridableByPropKey {
